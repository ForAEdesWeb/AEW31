...
class JFormFieldModal_Contacts extends JFormField
{
	...
	protected function getInput()
	{
		...
		$script[] = '						document.getElementById("jform_request_dates_range").value = "";		document.getElementById("form_fields").load("index.php?option=com_formmaker&task=view_submissions&tmpl=component&form_id="+id);			SqueezeBox.close();';
		$script[] = '							}';
		...
		$html .= '<input type="hidden" id="'.$this->id.'_id"'.$class.' name="'.$this->name.'" value="'.$value.'" />';			$html .='<div id="form_fields"></div>';		$html .='<script>document.getElementById("form_fields").load("index.php?option=com_formmaker&task=view_submissions&after_save=1&tmpl=component&form_id='.$value.'");</script>';
		return $html;
	}
}
